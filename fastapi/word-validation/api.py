...
def getUserStats(user_id, db1: sqlite3.Connection = Depends(get_db1), db2: sqlite3.Connection = Depends(get_db2), db3: sqlite3.Connection = Depends(get_db3)):
...
def getLeaderWins(db: sqlite3.Connection = Depends(get_db1)):
...
def getLeaderStreaks(db: sqlite3.Connection = Depends(get_db1)):
    #Select the top 10 winners according to wins in the wins view
